...
After reconnection, positions showed "db only" for a long time:
- Server needed up to 7 seconds to update cache (5s loop + 2s data)
- Dashboard only fetched positions every 2 minutes
- Even with immediate fetch, cache wasn't ready when login was detected

### Fix

1. **Cache before status**: Update cache BEFORE setting `logged_in=True`
   - Dashboard sees login only after cache is ready
2. **Faster dashboard refresh**: Reduced from 120s → 15s

**Order change in `_ensure_connected()`:**
```
Before:                          After:
1. Set logged_in=True            1. Setup (subscribe, callbacks)
2. Setup (subscribe, callbacks)  2. Update cache
3. Update cache                  3. Set logged_in=True
```

| Before | After |
|--------|-------|
| ~127 seconds worst case | Instant (cache ready when login detected) |

---

## Docker Image Cleanup

Removed ~130 old Docker images, saving ~58GB disk space.
