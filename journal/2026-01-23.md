...
5. `Remove subprocesses: executions in connection manager, delete debug endpoint` - cf5bb51
6. `Remove executions endpoint - commission already in trade_history` - 26bb480
...
- Caches all dashboard data:
  - SPY price (streaming, every 5 seconds)
  - Option prices and Greeks (streaming, every 5 seconds)
  - Open orders (every 5 seconds)
  - Positions with margin and P&L (every 5 seconds)
- readonly=False (needed for whatIfOrder margin calculation)
...
**Snapshot Function (16:05 daily):**
- Uses connection manager cache (no separate connection)
- Saves aggregated Greeks, margin, P&L to database

**Key benefits:**
- No more subprocess spawning
- Dashboard endpoints are instant (read from cache)
- Single IBKR connection for all reads

---

## Next Steps (for future sessions)

1. Deploy changes to EC2 (`git pull && docker-compose up -d --build`)
2. Monitor trading execution during market hours
3. Verify margin calculation works correctly with whatIfOrder
4. Consider adding last_update timestamp to dashboard UI
